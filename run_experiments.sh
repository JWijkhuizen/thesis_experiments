...
declare -a configs_sa=("teb_v1_a0_b0" "dwa_v1_a0_b0")
declare reconfiguration="false"

let runmax=${#configs[@]}+2
declare runid=0
...
declare fail=2
for config in ${configs_sa[@]} ; do
	fail=1
	while [ $fail -ne 0 ] ; do
		echo "${runid}/${runmax}"
		bash -ic "./run_single_sim.sh -n $config -r 'true';
		exit;"
		fail=$?
		if [ $fail -ne 0 ]; then
...
for config in ${configs[@]} ; do
	...
	while [ $fail -ne 0 ] ; do
		echo "${runid}/${runmax}"
		bash -ic "./run_single_sim.sh -n $config -r $reconfiguration;
		exit;"
		fail=$?
		if [ $fail -ne 0 ]; then
			echo "${fail}: Failed run, try again"
